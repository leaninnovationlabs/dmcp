...
async def get_all_users(db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        print(f"Users: {users}")
        return create_success_response(data=users)
    except Exception as e:
...
async def get_current_user(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        if not user:
            return JSONResponse(
                status_code=404,
                content=create_error_response(errors=["User not found"]).model_dump(),
            )
...
async def generate_token(request: Request, db: AsyncSession = Depends(get_db)):
...
async def delete_user(user_id: int, db: AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        return create_success_response(data={"message": "User deleted successfully"})
    except Exception as e:
        return JSONResponse(
            status_code=500,
            content=create_error_response(
...
async def add_role_to_user(user_id: int, role: str, db: AsyncSession = Depends(get_db)):
